...
export class DatabaseStorage implements IStorage {
  ...
  async getTrendingEvents(): Promise<Event[]> {
    return await db.select().from(events).where(eq(events.isTrending, true));
  }
  ...
  async updateEventSeats(id: number, seatsBooked: number): Promise<Event | undefined> {
    // First get the event to check available seats
    const [event] = await db.select().from(events).where(eq(events.id, id));
    if (!event) return undefined;
...
// Seed data function to populate the database with initial data
